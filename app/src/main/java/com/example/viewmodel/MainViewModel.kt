...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun performLocalSpatialOcr(
        ...
        viewModelScope.launch {
            try {
                ...
                if (spatialResult.lines.isEmpty() && spatialResult.rawText.isBlank()) {
                    val msg = "ছবিতে কোনো টেক্সট শনাক্ত করা যায়নি।"
    ...
    /**
     * Performs Cloud Google Drive OCR + 2D Spatial Bounding Box Layout Alignment.
     */
    fun performGoogleDriveOcr(
        bitmap: android.graphics.Bitmap,
        onResult: (Boolean, String, com.example.util.GoogleDriveOcrHelper.ParsedStudentInfo?, com.example.util.SpatialOcrEngine.SpatialAnalysisResult?) -> Unit
    ) {
        viewModelScope.launch {
            ...
            if (token.isNullOrBlank()) {
                ...
                onResult(false, errorMsg, null, null)
                return@launch
            }
            ...
            if (res.isFailure) {
                isOcrProcessing.value = false
                ocrProgressMessage.value = null
                val msg = res.exceptionOrNull()?.localizedMessage ?: "OCR ব্যর্থ হয়েছে"
                userMessage.value = msg
                onResult(false, msg, null, null)
                return@launch
            }

            val extractedText = res.getOrNull().orEmpty()
            ...
            ocrProgressMessage.value = "বাউন্ডিং বক্স ও লেবেল অবস্থান সমন্বয় করা হচ্ছে..."

            val spatialResult = try {
                com.example.util.SpatialOcrEngine.analyzeBitmap(
                    context = getApplication(),
                    bitmap = bitmap,
                    cloudOcrText = extractedText
                )
            } catch (_: Exception) {
                null
            }
            ...
            val parsedStudent = spatialResult?.formattedResult?.studentInfo
                ?: com.example.util.GoogleDriveOcrHelper.parseStudentFromOcrText(extractedText)

            userMessage.value = "Google Drive OCR ও বাউন্ডিং বক্স বিশ্লেষণ সফল হয়েছে!"
            onResult(true, extractedText, parsedStudent, spatialResult)
        }
    }
